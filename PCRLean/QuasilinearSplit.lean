...
noncomputable def canonicalRootCentre (a : σ → R) : Ideal P :=
...
theorem diagonalForm_mem_canonicalRootCentre_sq (a : σ → R) :
  ...
  exact Ideal.pow_mem_pow
    (linearRoot_mem_centre (canonicalRootCoeff a)) 2

/-- Hence the diagonal fully quasilinear mark-two packet is permissible for
its canonical root centre. -/
...
theorem canonicalRootCoeff_unique (a : σ → R) (i : σ) {b : R}
  ...
  simpa [frobeniusEquiv_def, canonicalRootCoeff] using hb
