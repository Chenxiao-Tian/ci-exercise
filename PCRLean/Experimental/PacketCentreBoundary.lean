...
A second counterexample separates strictness from marked permissibility: the
linear ideal `(X)` contains `X` but the mark-two packet generated by `X` is not
contained in `(X)^2`. Thus regularity, strictness, and marked legality are three
independent gates.

The statements below do not assert singular-locus containment, boundary
compatibility, passive safety, or hereditary blowup transport.
-/
...
section MarkedBoundary

/-- A strict-looking linear support centre can still fail the marked-power
gate. The generator `X` lies in `(X)` but not in `(X)^2`, so the mark-two packet
`((X), 2)` is not permissible for `(X)`. -/
theorem generatorCentre_not_markTwo_permissible :
    ¬ MarkedIdeal.Permissible
      (R := Polynomial ℤ)
      ⟨Ideal.span {Polynomial.X}, 2, by omega⟩
      (Ideal.span {Polynomial.X}) := by
  intro hperm
  have hx :
      Polynomial.X ∈
        (Ideal.span {Polynomial.X} : Ideal (Polynomial ℤ)) ^ 2 :=
    hperm (Ideal.mem_span_singleton_self Polynomial.X)
  rw [Ideal.span_singleton_pow] at hx
  exact MarkedIdeal.generator_support_not_square_span.2 hx
