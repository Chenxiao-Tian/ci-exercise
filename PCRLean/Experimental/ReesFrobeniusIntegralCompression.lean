...
this one root is both integral and of finite type, hence finite as an `S`-module.
Thus Frobenius mark compression is naturally an integral-closure operation
rather than an identity blowup.
...
/-- The root-adjoin extension is finite as a module: integral plus finite type. -/
theorem rootAdjoin_moduleFinite
  ...
  letI : Algebra.IsIntegral S
      (Algebra.adjoin S {weightedTerm m g}) :=
    rootAdjoin_isIntegral p S m f g hroot hsource
  letI : Algebra.FiniteType S
      (Algebra.adjoin S {weightedTerm m g}) :=
    rootAdjoin_finiteType S m g
...
theorem weightedRootAdjoin_moduleFinite
    ...
    Module.Finite
      ...
      (Algebra.adjoin
        (Algebra.adjoin A {weightedTerm (m * p) f})
        {weightedTerm m g}) := by
  apply rootAdjoin_moduleFinite p
  exact Algebra.subset_adjoin rfl
