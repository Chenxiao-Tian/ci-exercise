...
Merges must preserve the unused ledger and lower the number of active support
blocks.  Internal cleanup preserves both outer coordinates and lowers a local
height.  Thus the abstract `GenRank` is now computed from actual source data,
rather than supplied as arbitrary natural numbers.
...
/-- One finite causal-ledger stage.  `active` stores the actual support blocks
of all currently active identities. -/
...
/-- The concrete causal rank extracted from the ledger. -/
...
/-- No infinite execution can satisfy the actual finite-source ledger
protocol. -/
