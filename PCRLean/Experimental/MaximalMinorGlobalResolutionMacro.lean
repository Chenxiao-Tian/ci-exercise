...
* equality of that centre with the ideal generated by all original packet
  equations `Σ_j M_{rj} Z_j - b_r`;
...
Thus the centre no longer appears only after local minor choices: it is already
the actual ideal generated by the original finite rectangular packet.

This is the strongest current affine compiler from presentation-level Fitting
data to one actual ordinary-centre step. The remaining universal bridge is
extraction of the rectangular packet and the two determinantal identities from
an arbitrary intrinsic Frobenius--Hasse state.
-/
...
/-- The actual ideal generated directly by all rows of the original rectangular
packet. -/
def fullPacketIdeal : Ideal (MvPolynomial ι R) :=
  MaximalMinorEquationIdeal.equationIdeal M b

/-- The original full packet ideal is exactly the canonical global centre. -/
theorem fullPacketIdeal_eq_globalCentre :
    fullPacketIdeal M b = globalCentre M b Hrank Haug := by
  exact MaximalMinorEquationIdeal.equationIdeal_eq_graphIdeal
    M b (Haug.globalGraph Hrank)
...
noncomputable def certificate
  ...
  fullPacketIdeal := fullPacketIdeal M b
  globalCentre := globalCentre M b Hrank Haug
  centre_eq := rfl
  packetIdeal_eq_centre := fullPacketIdeal_eq_globalCentre
    M b Hrank Haug
  globalMacro :=
    FittingAugmentedMinorGlobalResolutionMacro.certificate
      (R := R) (N := N)
...
/-- Every selected subsystem ideal is also the base change of the ideal
actually generated by the complete original packet. -/
theorem allLocalSystemIdeals_fromFullPacket
    (s : Selection (κ := κ) (ι := ι)) :
    ((MaximalMinorCommonPacket.data M b Hrank).toSystemAtlas.localSystem s)
        .equationIdeal =
      Ideal.map
        (PolynomialGraphBaseChange.polynomialMap
          ...
          (algebraMap R
            (FittingGraphCentreAtlas.ChartRing Hrank.toAtlas s)))
        (fullPacketIdeal M b) := by
  rw [fullPacketIdeal_eq_globalCentre M b Hrank Haug]
  exact allLocalSystemIdeals_fromGlobal M b Hrank Haug s
