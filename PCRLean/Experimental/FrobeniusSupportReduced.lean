...
/-!
# Frobenius support heredity over reduced coefficient rings

The support-scaling argument for a prime-power Frobenius power does not require
an integral domain.  The exact necessary hypothesis is that nonzero
coefficients cannot become zero after a positive power, namely reducedness.

For a reduced commutative ring `A` of characteristic `p`, raising a
multivariate polynomial to `p^e` scales every supported exponent by `p^e` and
raises every coefficient to the same power.  Reducedness prevents a supported
coefficient from disappearing.  Consequently the support-order lower-bound
predicate satisfies

`OrderGE (g^(p^e)) ((p^e)*m) ↔ OrderGE g m`.

This is the coefficient-ring form needed for normal cones: after a regular
centre is locally written as a coordinate or graph centre, the passive
coordinate ring need only be reduced, not a domain.
-/

namespace PCRLean
namespace Experimental
namespace FrobeniusSupportReduced
...
/-- A nonzero element of a reduced ring stays nonzero under every positive
power. -/
theorem pow_ne_zero_of_isReduced
    {x : A} (hx : x ≠ 0) {n : Nat} (hn : 0 < n) :
    x ^ n ≠ 0 := by
  intro hpow
  by_cases h1 : n = 1
  · subst n
    exact hx (by simpa using hpow)
  · have hlt : 1 < n := by omega
    have hzero : x = 0 :=
      ((isReduced_iff_pow_one_lt n hlt).mp
        (inferInstance : IsReduced A)) x hpow
    exact hx hzero
...
/-- Every supported root monomial survives at its scaled exponent. -/
theorem scaled_mem_support
  ...
  have hcoeff : (MvPolynomial.coeff d g) ^ (p ^ e) ≠ 0 :=
    pow_ne_zero_of_isReduced
      (MvPolynomial.mem_support_iff.mp hd) hq
  ...
  · intro b hb hbd
    ...
    intro hscale
    apply hbd
    exact scaleExponent_injective (σ := σ) (p ^ e) hq hscale
  · intro hdnot
    exact (hdnot hd).elim
...
/-- Frobenius power and scaled mark preserve support order exactly over every
