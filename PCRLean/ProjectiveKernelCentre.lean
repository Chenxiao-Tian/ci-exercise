...
section.  Combined with `FiniteKernelCentre`, this turns the persistent kernel
into a direct summand without choosing a section as extra geometric data.
...
/-- A surjective finite evaluation packet admits a section because its finite
function-space target is free, hence projective. -/
theorem exists_splitPacket_of_surjective
  ...
  rcases LinearMap.exists_rightInverse_of_surjective
      (evaluationMap pair s) hrange with ⟨section, hsection⟩
...
theorem exists_complement_of_surjective
    ...
    ∃ section : ((d : s) → K) →ₗ[K] V,
      IsCompl (LinearMap.ker (evaluationMap pair s))
        (LinearMap.range section) := by
  rcases exists_splitPacket_of_surjective pair s hsurj with ⟨P⟩
  exact ⟨P.section, P.isCompl_kernel_range_section⟩
...
theorem persistentKernel_isCompl_of_surjective
  ...
  exact ⟨S.section, P.persistentKernel_isCompl S⟩
